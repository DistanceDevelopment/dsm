...
# parametric moving block bootstrap to obtain variance
param.movblk.variance <- function(B, dsm.object, pred.object, 
                                  samp.unit.name='Transect',block, 
                                  field=FALSE, cell.name, bpfile){

# B=num.boot -- number of resamples
# dsm.object=dsm.object -- object returned from dsm.fit()
# pred.object=predict.object -- this used to be more complicated, for now
#                          just a data frame as would usually get passed to 
#                          predict.gam() 
# samp.unit.name='Transect' -- name of the (re)sampling unit
# block=num.segs.in.block
# field=FALSE [[this and cell.name are linked, see below]]
# cell.name=cell [[size of the cell to convert density to abundance]]
# bpfile -- file used by distance to get the progress bar -- REMOVE?
#           ignored at the moment
  ...
  block.info <- block.info.per.su.f(block=block,
  ...
  for(i in 1:B){
    ...
    bs.resids <- generate.mb.sample.f(k = k, len = block, 
...
# sort out the block information for each sampling unit
block.info.per.su.f <- function(block,data,name.su){
# block=block.size,data=dsm.data,name.su=unique.sampling.units
  unit <- NULL   
  unit$name <- name.su
...
generate.mb.sample.f <- function(k, len, which.blocks, x, unit.info, n.units){
