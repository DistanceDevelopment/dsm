...
# Created 8/9 February, 3, 13 April, 27 June 2006 by Rexstad
  ...
  # Following statement does not work, because ddf data names segment labels 
  # "label", not "Sample.label" as in the var.dat.r file; hence cannot merge 
  # upon the specified by=
  #  ddf.data <- merge(eval(parse(text=ddfdata.name)), vardata.in, 
  #                             by="Sample.Label")

  # Attach 'transect' to data used for the dsm object
  dsmdata.name <- paste("dsm.", dsm.ID, "$result$data", sep="")
  ready.to.overwrite <- merge(eval(parse(text=dsmdata.name)), 
                                vardata.in, by="Sample.Label")
  ...
  predict.object.name <- eval(parse(text=paste("dsm.predict.", 
                                               prediction.ID,sep="")))
  
  # Call appropriate bootstrapping function, based on the value of 'method'
  # I'm thinking about not allowing writing to a file within the functions, 
  # or at least not writing to the file reserved for containing the 
  # cell-by-cell SEs.
  
  # Set working directory to the temp directory defined by Distance
  # This will enable writing the bootstrap counter to simply write a 
  # filename, without need to know the path, or pass the pathname to 
  # Louise's functions
  current.working.dir <- getwd()
  setwd(path)  

  if(method=="movblock"){
    bootstraps <- param.movblk.variance(dsm.object=new.dsm.object,
                                        pred.object=predict.object.name,
                                        samp.unit.name='Transect',
                                        B=replicates,
                                        block=blocksize, 
                                        field=field.flag, 
                                        cell.name=cell.area,
  ...
  #                      'study.area.total' holding replicate-specific densities
    ...
    #bootstraps <- dsm.variance.nonpar.f(ddf.object=ddf.object.name, 
    #                                    dsm.object=dsm.object.name,
    #                                    grid=grid.object.name, 
    #                                    samp.unit.name='Transect', 
    #                                    B=replicates)
  }
  # Send standard errors for each cell to a file for subsequent read into GIS 
  # layer having applied machine formula with sufficient statistics contained 
  # in the return values from call to 'variance.par.mb.f'
  machine.var <- (bootstraps$short.var$sumx.sq - 
                  (bootstraps$short.var$sumx^2/replicates)) / (replicates-1)
  vector.cell.se <- sqrt(machine.var)
  
  # Done with the bootstrapping, restore correct working directory for 
  # remainder of function
  setwd(current.working.dir)
  write.table(vector.cell.se, file=fname, quote=FALSE, 
              col.names=FALSE, sep="\t")
  ...
  # within 8% of each other these are defined as 'outliers,' and will be 
  # printed to note their existence, but removed from all subsequent 
  # computations
  ...
  # vector.abund.studyarea <- apply(bootstrap.abund,MARGIN=2,sum,na.rm=TRUE)
  # sum over cells
  print(cat('\tResponse surface/Variance Plot:  Bootstrap distribution\t\n'))
  hist(bootstrap.abund[is.finite(bootstrap.abund)], 
       xlab="Estimated values", 
      main=paste("Distribution of bootstrap estimates (after trimming largest",
                 round(trim*100,0), "%)"))
  ...
  # fixed bug ER 06/02/2008 15:31:33--"summary.ds" replaced by 
  #  "summary" next 2 lines
  ...
  # Other calculations to perform if there are subareas within study area 
  # of interest  

  # Problematic because although dsm.layers.X still resides within .Rdata, 
  # there no longer remains the labels associated with them.  They were in 
  # a temporary file that evap.

  # Construct name of layer objects, from prediction.id
  layer.object.name <- eval(parse(text=paste("dsm.layers.", prediction.ID,sep="")))
  
  # There may be multiple subarea layers over which abundance is being 
  # estimated, and hence we need to perform  'aggregate' for each of 
  # those layers
  num.of.sublayers <- length(colnames(layer.object.name))
  if(num.of.sublayers != 2){
    # I suggest substituting 'machine.var' for 'bootstrap.abund'
    bootstrap.abund <- data.frame(bootstrap.abund, layer.object.name)  
    for(sublayer in 2:num.of.sublayers){
      # Test for layer in which all ids are zero (meaning this is 
      # the study area layer)
      if(sum(layer.object.name[,sublayer]) != 0){
        subunit.sums <- aggregate(bootstrap.abund, 
                                  list(ID.name=layer.object.name[,sublayer]), 
                                  sum, na.rm=TRUE)
        # Eliminate the sum for cells with ID.name=0, meaning they were 
        # not in a subunit of interest
        subunit.sums <- subset(subunit.sums, ID.name!=0)
        # Eliminate 3 extraneous columns, i.e,  ID, ID.name and final 
        # column (with user-specified name) 
        # Trickery is needed to determine name of final column, and 
        # pass this to subset to remove
        subunit.columns <- length(colnames(subunit.sums))
        subunit.sums <- subset(subunit.sums, select=-c(ID.name, 
                                (subunit.columns-1):subunit.columns))  
  ...
  dsm.bootstrap.pre.post <- list(
       lower.abund.ci=quantile(bootstrap.abund[is.finite(bootstrap.abund)], 
                                (1-alpha)/2,na.rm=TRUE), 
       upper.abund.ci=quantile(bootstrap.abund[is.finite(bootstrap.abund)], 
                                1-(1-alpha)/2,na.rm=TRUE),
