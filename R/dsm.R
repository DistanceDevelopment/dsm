...
#' \tabular{ll}{
#'   \code{n}, \code{count}, \code{N} \tab count in each segment. Only covariates that vary at the segment level can be used in the detection function when using this response.\cr
#'   \code{Nhat}, \code{abundance.est} \tab estimated abundance per segment, estimation is via a Horvitz-Thompson estimator. This should be used when there are covariates in the detection function at the observation level.\cr
...
dsm <- function(formula, ddf.obj, segment.data, observation.data,
  ...
  # if we on;t have one detection function, but the model was
  ...
  possible.responses <- c("density", "density.est",
                          "count",
                          "abundance.est")
  ...
  # throw an error for old responses
  old.responses <- c("D", "Dhat", "N", "n", "Nhat")
  if(response %in% old.responses){
    stop("Short response variable names are no longer supported, please update your formula")
  }
  ...
  if(!(response %in% c("D", "density", "Dhat", "density.est"))){
    ...
    # set the weights if we are doing density
    if(is.null(weights)){
